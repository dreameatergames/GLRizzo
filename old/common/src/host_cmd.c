...
void Host_Startdemos_f (void)
{
	...
	//		Con_Printf (va("startdemos: couldn't find %s\n", name));
		...
		fclose (f);
		f = NULL;
		strncpy (cls.demos[i-i2-1], Cmd_Argv(i), sizeof(cls.demos[0])-1);
	}
	democount = c-i2;
	// Manoel Kasimier - end

//	for (i=1 ; i<c+1 ; i++) // Manoel Kasimier - removed
//		strncpy (cls.demos[i-1], Cmd_Argv(i), sizeof(cls.demos[0])-1); // Manoel Kasimier - removed
...
void Host_InitCommands (void)
{
	...
	Cmd_AddCommand ("del", Host_Delete_f); // Manoel Kasimier
