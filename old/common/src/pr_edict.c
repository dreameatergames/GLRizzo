...
====================
*/
char		prev_ed_name[256]; // Manoel Kasimier
char		badfields[256*4]; // Manoel Kasimier
char *ED_ParseEdict (char *data, edict_t *ent)
{
	...
	badfields[0] = 0; // Manoel Kasimier - clear it
...
if (!strcmp(com_token, "angle"))
{
	...
	anglehack = false;
	// Manoel Kasimier - rotating brushes - begin
	...
	// Manoel Kasimier - rotating brushes - end
...
// and are immediately discarded by quake
		...
		if (!key)
		{
			Q_strcat(badfields, va("'%s' is not a field\n", keyname)); // Manoel Kasimier
		//	Con_Printf ("'%s' is not a field\n", keyname); // Manoel Kasimier - removed
			continue;
		}
...
{	
	...
	prev_ed_name[0] = 0; // Manoel Kasimier - clear it
// parse ents
	while (1)
	{
...
//
	...
	// look for the spawn function
		...
		if (!func)
		{
			...
			ED_Print (ent);
		//		if (badfields[0]) Con_Printf(badfields); // Manoel Kasimier
			} // Manoel Kasimier
			ED_Free (ent);
			continue;
		}
...
// 2001-09-12 Returning information about loaded file by Maddes  end
	...
	Con_DPrintf ("progs.dat occupy %iK.\n", com_filesize/1024); // edited
