...
cvar_t	gl_polyblend = {"gl_polyblend", "1", false}; // Manoel Kasimier - gl_polyblend
int	gl_polyblend_old; // Manoel Kasimier - gl_polyblend
...
// drop the bonus value
	...
	force = V_CheckGamma ();
// Manoel Kasimier - gl_polyblend begin
...
/* Manoel Kasimier - gl_polyblend end
	if (!new && !force)
		return;
// Manoel Kasimier - gl_polyblend */
	...
	for (i=0 ; i<256 ; i++)
	{
		...
		if (gl_polyblend.value != 0) // Manoel Kasimier - gl_polyblend
		for (j=0 ; j<NUM_CSHIFTS ; j++)	
		{
...
//			Con_Printf("%f\n", dmg);
	...
	// Manoel Kasimier - begin
	// � menor que zero, ent�o mudamos o v_dmg_time pra -666
	// pra que o Vibration_Stop s� seja ativado neste frame
...
//		r_refdef.vrect.height >>= (r_framecount&1); // Manoel Kasimier
...
void V_Init (void)
{
	...
	Cvar_RegisterVariable (&gl_polyblend); // Manoel Kasimier - gl_polyblend
