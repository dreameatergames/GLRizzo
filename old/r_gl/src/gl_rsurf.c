...
void R_DrawSequentialPoly (msurface_t *s)
{
	...
	*/	// BlackAura - removed - end
	...
	if (s->flags & SURF_DRAWTURB)
	{
	//	GL_DisableMultitexture(); // BlackAura - removed
		GL_Bind (s->texinfo->texture->gl_texturenum);
		EmitWaterPolys (s);
		return;
	}
...
void R_BlendLightmaps (void)
{
	...
	for (i=0 ; i<MAX_LIGHTMAPS ; i++)
	{
		...
		if (lightmap_modified[i])
		{
			lightmap_modified[i] = false;
			theRect = &lightmap_rectchange[i];
			/* @TODO: uncommented the comments, commented the uncommented */
...
//	if (!gl_texsort.value) // BlackAura - removed
//		GL_SelectTexture(TEXTURE1_SGIS); // BlackAura - removed
...
// 	if (!gl_texsort.value) // BlackAura - removed
//		GL_SelectTexture(TEXTURE0_SGIS); // BlackAura - removed
