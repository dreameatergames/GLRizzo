...
cvar_t		gl_max_size = {"gl_max_size", "256"}; // 1024 BlackAura
...
#if 0 //#ifdef _WIN32 // BlackAura - edited
...
{
#ifndef _arch_dreamcast // BlackAura - We don't use any 32-bit textures on the Dreamcast
	int			samples;
...
// BlackAura - end
	...
	if (scaled_width == width && scaled_height == height)
	{
		if (!mipmap)
		{
			glTexImage2D (GL_TEXTURE_2D, 0, internal_fmt, scaled_width, scaled_height, 0, GL_COLOR_INDEX , GL_UNSIGNED_BYTE, data); // BlackAura - edited
			goto done;
		}
	...
	glTexImage2D (GL_TEXTURE_2D, 0, internal_fmt, scaled_width, scaled_height, 0, GL_COLOR_INDEX, GL_UNSIGNED_BYTE, scaled); // BlackAura - edited
	if (mipmap)
	{
		int		miplevel;
		...
		while (scaled_width > 1 || scaled_height > 1)
		{
			...
			if (scaled_height < 1)
				scaled_height = 1;
			miplevel++;
			glTexImage2D (GL_TEXTURE_2D, miplevel,internal_fmt, scaled_width, scaled_height, 0, GL_COLOR_INDEX, GL_UNSIGNED_BYTE, scaled); // BlackAura - edited
		}
	}
done: ;
// BlackAura - begin
...
// BlackAura - begin
	...
	GL_Upload8_EXT (data, width, height, false, alpha);
#else
...
{
	qboolean noalpha;
	int   i, p, s, lhcsum; // BlackAura - edited
	gltexture_t *glt;
