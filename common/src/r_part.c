		...
		for (j=-16 ; j<16 ; j++)
			{
				...
				p->start_time = cl.time+1.6; // Manoel Kasimier
				p->color = 224 + (rand()&7);
				p->type = pt_slowgrav;
				...
				VectorNormalize (dir);						
				vel = 150 + (rand()&63);//50 + (rand()&63); // Manoel Kasimier - edited
				VectorScale (dir, vel, p->vel);
			}
}
...
//		D_DrawParticle (p);
	...
	if (r_particlealpha.value && alphamap)
	{
//		if (p->color < 96) // 192/2
//		{
...
#endif
		case pt_slowgrav:
			p->vel[2] -= grav*4; // Manoel Kasimier - edited
			break;
		}
//		if (p->die == -1) p->start_time = -2; // Manoel Kasimier
	}
